...
in `../validation.md`: unrecognised keys in a named extensions object
where a document type defines one. Unknown `x-ctier-*` headers are
the namespace strip in `../headers.md`, a different rule.
...
| `decision-record-0.2.0.json` | An operation was classified and acted upon. Carries `deploymentDigest`. `declaredTier` is optional. |
| `attempt-record-0.3.0.json` | The enforcement point refused without classifying. Carries `deploymentDigest`. `declaredTier` is present when a declaration was made and absent when it was not. |

...
The API owner, at design time, declared in the OpenAPI spec as an extension field. The platform enforces at runtime. Agents never declare their own tier — that is C11, and it is the same mistake as letting a caller assert its own scopes. Strip, integrity-protection, and a deploy-time binding that never reads an inbound tier header are named ways to meet it. An operation in the composed description with no declared tier is Tier 4 by default, so failing to classify is fail-closed rather than a silent gap. It is not a default for a path the composition does not contain.

**Does the gateway evaluate the five criteria on every request?**
No, and this is the distinction that makes the latency budget work. Classification happens twice, in different senses. At design time a human runs the five criteria and writes the result into the spec — judgement work, done once, reviewed like any other change. At runtime the reference **resolves** that declared tier from configuration compiled with the route — a lookup rather than an evaluation — and then applies the three things the specification could not have known: how much consequence the session has already accumulated, whether the delegation chain shows authority expansion, and whether the actual payload exceeds what the declaration assumed. A declared Tier 1 bounded query arriving with a limit of ten thousand is not the operation that was classified. Lookup is how this implementation meets C12; a request-time check against a loaded description is the other permitted mechanism. C12 does not require a generator.
...
Exclusion and tiering operate at different layers, and conflating them is the source of the confusion. Exclusion is an authorisation decision: the agent's client never holds the scope, so the request is refused at token validation, before the gateway looks for a consequence tier. There is nothing to classify because nothing reached classification. The undeclared-equals-Tier-4 default is a consequence decision, and it applies only to operations that are in the composed description and that the agent can already reach within a scope it legitimately holds. It is the safety net for a classification somebody missed, not a default for a path the composition does not contain, and not a statement about endpoints that were never granted. **Exclude at the scope; tier within it.**
...
Severity is what decides the tier, so yes — reversibility, blast radius and regulatory exposure are what push an operation from 3 to 4. But the control itself differs in kind, not degree. In Tier 3 the human is genuinely in the middle: the agent's payload, executed under the policy service's identity, with the originating agent and the authorising person recorded as attribution, and the human contributing a yes or no to a pre-formed action. In Tier 4 there is no middle, because the agent has been removed from the path — there is no held operation waiting for approval, and the human constructs the action themselves, in another system, under their own credential.

That matters for two reasons. First, Tier 4 exists for cases where the risk is not that the agent acts without permission but that the agent's *construction* of the action is wrong in a way a reviewer will not catch. Approving an agent's payment payload does not protect you; it launders the decision through someone who is reading rather than deciding. Second, attribution: in Tier 3 a named person authorised an action; in Tier 4 a named person performed one. "A clinician authorised what the agent proposed" is a materially weaker position than "the clinician ordered it," legally and in an incident review. Many regulated processes require a licensed human to be the actor rather than the approver.
...
Two properties, and you need both. C5: the withheld response is terminal and instructs the agent to continue without this step — the `202` field list is one way to say that. C7: a duplicate attempt MUST NOT produce a second execution — a derived idempotency key is one way. This failure mode is what kills naive synchronous approval designs: a control built to prevent an operation causes it to happen twice.

**Why persist the request instead of asking the agent to resubmit?**
Because the agent may re-plan between the pending response and the approval. Resubmission means you approved one payload and executed another. C6 is those two properties: the bytes that execute are the bytes that were approved, and the agent is not the client. Persisting the request is how the reference meets it; a signed snapshot the agent never resubmits is the other named mechanism. The policy service is the client; the agent's credential is not stored and not replayed.
...
**Leaving the unclassified unhandled.** An operation in the composed description with no declared tier must be Tier 4 until somebody classifies it. If unclassified defaults to permitted, your coverage gaps are invisible to you and they grow every sprint. A path outside the composition is not this default.

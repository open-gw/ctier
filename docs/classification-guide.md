...
The API owner, at design time, declared in the OpenAPI spec as an extension field. The platform enforces at runtime. Agents never declare their own tier — that is C11, and it is the same mistake as letting a caller assert its own scopes. Strip, integrity-protection, and a deploy-time binding that never reads an inbound tier header are named ways to meet it. An endpoint with no declared tier is Tier 4 by default, so failing to classify is fail-closed rather than a silent gap.
